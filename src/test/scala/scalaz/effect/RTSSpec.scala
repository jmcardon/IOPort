...
class RTSSpec(implicit ee: ExecutionEnv)
  ...
  def testEvalOfAttemptOfSyncEffectError =
    unsafePerformIO(IO.sync(throw ExampleError).attempt) must_=== -\/(
      ExampleError)

  def testEvalOfAttemptOfFail = {
    unsafePerformIO(IO.fail[Int](ExampleError).attempt) must_=== -\/(
      ExampleError)

    unsafePerformIO(IO.suspend(IO.suspend(IO.fail[Int](ExampleError)).attempt)) must_=== -\/(
      ExampleError)
  }
  ...
  def testEvalOfUncaughtThrownSyncEffect =
    unsafePerformIO(IO.sync[Int](throw ExampleError)) must (throwA(
      ExampleError))
  ...
  def testEvalOfFailEnsuring = {
    ...
    unsafePerformIO(
      IO.fail[Unit](ExampleError)
        .ensuring(IO.sync[Unit] { finalized = true; () })) must (throwA(
      ExampleError
    ))
  ...
  def testEvalOfFailOnError = {
    ...
    unsafePerformIO(
      IO.fail[Unit](ExampleError)
        .onError(_ => IO.sync[Unit] { finalized = true; () })) must (throwA(
      ExampleError
    ))
  ...
  def testBracketRethrownCaughtErrorInAcquisition = {
    lazy val actual = unsafePerformIO(
      IO.absolve(
        IO.fail[Unit](ExampleError).bracket_(IO.unit)(IO.unit).attempt))
  ...
  def testBracketRethrownCaughtErrorInRelease = {
    lazy val actual = unsafePerformIO(
      IO.absolve(
        IO.unit.bracket_(IO.fail[Unit](ExampleError))(IO.unit).attempt))
  ...
  def testBracketRethrownCaughtErrorInUsage = {
    lazy val actual = unsafePerformIO(
      IO.absolve(
        IO.unit.bracket_(IO.unit)(IO.fail[Unit](ExampleError)).attempt))
